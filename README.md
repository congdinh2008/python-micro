...
```
    ...
    └─────┬──────┘      └────────┬────────┘      └──────────-┬────────┘
          ...
          │                      │           │               │
    ┌─────▼──────┐      ┌────────▼────┐  ┌──-▼─────┐  ┌────--▼────────┐
    │  User DB   │      │ Product DB  │  │  Redis  │  │   Order DB    │
...
│  │  (Metrics)  │   │(Visualize)│   │ (Logs) │   │(Collector)│   │
│  │   :9090     │   │  :3000    │   │ :3100  │   │           │   │
│  └─────────────┘   └──────────-┘   └────────┘   └──────────-┘   │
│                                                                 │
│  ┌─────────────┐   ┌──────────────────┐                         │
│  │   Jaeger    │   │ Redis Exporter   │                         │
│  │  (Tracing)  │   │   (Metrics)      │                         │
│  │   :16686    │   │     :9121        │                         │
│  └─────────────┘   └──────────────────┘                         │
│                                                                 │
...
- **Database per Service**: User DB, Product DB, Order DB (PostgreSQL)
- **Event-Driven**: RabbitMQ for async communication between services
- **Caching Strategy**: Redis for Product Service (Cache-aside pattern)
- **Authentication**: Centralized in User Service, JWT token validation
